<?php
	...
	foreach ($files as $i => &$file):
		...
		if ($thumb_data !== FALSE)
		{
			...
			$thumb_html = '<div class="assets-thumb"></div>';
		}
		else
		{
...
?>
	<li data-id="<?php echo $file->file_id() ?>" data-folder="<?php echo $file->row_field('folder_id') ?>" data-file-url="<?php echo $file->url() ?>" data-file_name="<?php echo $file->filename() ?>" class="assets-tv-file <?php echo $file_class ?><?php if ($thumb_size == 'large'): ?> assets-tv-bigthumb<?php endif ?><?php if (in_array($file->file_id(), $disabled_files)): ?> assets-disabled<?php endif ?><?php if ($file->selected): ?> assets-selected<?php endif ?>">
		<?php echo $thumb_html ?>
