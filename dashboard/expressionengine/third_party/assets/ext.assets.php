...
class Assets_ext
{
		...
		if ($query_result)
		{
			...
			if ($all_assets_fields)
			{
				...
				if ($tmpl_assets_fields)
				{
					...
					$draft_status = (int) (isset($this->EE->session->cache['ep_better_workflow']['is_draft']) && $this->EE->session->cache['ep_better_workflow']['is_draft']);
	...
	{
		if (isset($this->cache['filemanager_extension_ignore_files'][$data['upload_location_id'].$data['file_name']]))
		{
			return;
		}
