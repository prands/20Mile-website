...
class Assets_mcp
{
	...
	function __construct()
	{
		...
		if (REQ == 'CP')
		{
			...
			if ($this->EE->session->userdata['group_id'] == 1)
			{
				...
				$nav['s3_sources'] = BASE.AMP.$this->base.AMP.'method=sources';
	...
	function create_folder()
	{
		...
		try
		{
			$output = $this->EE->assets_lib->create_folder($path);
		}
		catch (Exception $exception)
		{
		...
		exit($this->EE->javascript->generate_json($output, TRUE));
	}
	...
	function move_folder()
	{
		...
		$new_parents = $this->EE->input->post('new_path');
	...
	function move_file()
	{
		...
		$old_ids = $this->EE->input->post('old_id');
		$new_paths = $this->EE->input->post('new_path');
		$actions = $this->EE->input->post('action');
		...
		{
			$old_ids = array($old_ids);
			$new_paths = array($new_paths);
			$actions = array($actions);
		}

		try
		{
			$output = $this->EE->assets_lib->move_file($old_ids, $new_paths, $actions);
			if (! $batch_mode)
			{
				$output = $output[0];
			}
	...
	function get_session_id()
	{
		...
		exit($this->EE->javascript->generate_json(array('session' => $session), TRUE));
	}
	...
	*/
	...
	function sources()
	{
		...
		$this->_set_page_title(lang('s3_sources'));

		$vars['base'] = $this->base;

		$s3_accounts = $this->EE->db->get_where('assets_sources', array('source_type' => 's3'))->result();
		$vars['s3_accounts'] = $s3_accounts ? $s3_accounts : array();
	...
	public function edit_source()
	{
		...
		if (!empty($source_id) && is_numeric($source_id))
		{
			try
			{
				$source = $this->EE->assets_lib->instantiate_source_type((object) array('source_type' => 's3', 'source_id' => $source_id));
		...
		$vars['source'] = $source;
		$vars['base'] = $this->base;
		$vars['setting_fields'] = $this->EE->assets_lib->get_source_settings_field_list('s3');
	...
	public function save_source()
	{
		...
		if ( !empty($name))
		{
			...
			$save_data = array();
			$field_list = $this->EE->assets_lib->get_source_settings_field_list('s3');
			foreach ($field_list as $field)
			{
				$save_data[$field] = $this->EE->input->post($field);
			}

			$save_data['bucket'] = $this->EE->input->post('bucket');
			$save_data['url_prefix'] = $this->EE->input->post('bucket_url_prefix');
			$save_data['location'] = $this->EE->input->post('bucket_location');

			$source_id = $this->EE->input->post('source_id');

			$data = array(
				'name' => $name,
				'settings' => json_encode((object) $save_data)
			);

			$this->EE->assets_lib->store_source('s3', $source_id, $data);

			// change the name for the top level folder as well
			$this->EE->assets_lib->rename_source_folder($source_id, 's3', $name);
	...
	public function get_s3_buckets()
	{
		...
		$key_id = $this->EE->input->post('access_key_id');
		$secret_key = $this->EE->input->post('secret_access_key');
		...
		exit ($this->EE->load->view('mcp/s3_buckets', $vars, TRUE));
	}
