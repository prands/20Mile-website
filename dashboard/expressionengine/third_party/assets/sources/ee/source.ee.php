...
class Assets_ee_source extends Assets_base_source
{
	...
	public function __construct($source_id, $filedir_prefs)
	{
		parent::__construct();

		$filedir_prefs = self::apply_filedir_overrides($filedir_prefs);

		// if this is an ACTION call, check if we can do stuff with this EE filedir
		if (REQ == 'ACTION'
			&& (
				(!$this->EE->session->userdata('member_id') OR $this->EE->session->userdata('is_banned'))
				OR in_array($source_id, $this->_get_denied_filedirs()))
			)
		{
	...
	private function _get_filedir_prefs($filedirs = 'all', $site_id = NULL)
	{
		...
		if ($run_query)
		{
			...
			if (REQ != 'PAGE' && ($denied = $this->_get_denied_filedirs()))
			{
	 ...
	 */
	private function _get_denied_filedirs()
	{
	...
	protected function _delete_source_folder($server_path, $source_row)
	{
		...
		foreach ($files as $file)
		{
			if (is_dir($file))
			{
				$this->_delete_source_folder($file, $source_row);
			}
			else
			{
				$this->_delete_source_file($file, $source_row);
			}
		}
	...
	protected function _delete_source_file($server_path, $source_row)
	{
		if (@unlink($server_path))
		{
			...
			$this->EE->db->where('upload_location_id', $source_row->filedir_id)
				->where('rel_path', $server_path)
				->delete('files');

			$this->_delete_thumbnails($server_path, $source_row->filedir_id);
	 ...
	 */
	public function get_folder_server_path($path, $source_row)
	{
		$filedir_prefs = $this->get_filedir($source_row->filedir_id);
	...
	protected function _do_upload_in_folder($folder_data, $temp_file_path, $original_name)
	{
		...
		if ($filedir->max_size && sizeof($temp_file_path) > $filedir->max_size)
		{
		...
		if (file_exists($file_path)
			OR $this->EE->assets_lib->get_file_id_by_folder_id_and_name($folder_data->folder_id, $original_name)
		)
		{
	...
	private function _create_thumbnails($image_path, $upload_folder_id)
	{
		...
		$preferences['server_path'] = self::resolve_server_path($preferences['server_path']);
