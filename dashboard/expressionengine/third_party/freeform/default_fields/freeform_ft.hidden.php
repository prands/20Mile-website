...
class Hidden_freeform_ft extends Freeform_base_ft
{
	...
	{
		//max field length
		$default_data	= trim((string) $this->EE->input->get_post('default_data'));
	...
	protected function parse_specials ($data = '')
	{
			...
			preg_match_all('/' . LD . 'segment_([0-9]+)' . RD . '/sm', $data, $matches, PREG_SET_ORDER))
		{
			...
			foreach ($matches as $match)
			{
				...
				{
					if (isset($this->EE->uri->segments[$match[1]]))
					{
						$variables['segment_' . $match[1]] = $this->EE->uri->segments[$match[1]];
					}
					else
					{
			...
			$data = $this->EE->TMPL->parse_variables($data, array($variables));
			$data = $this->EE->TMPL->advanced_conditionals($data);
		}
