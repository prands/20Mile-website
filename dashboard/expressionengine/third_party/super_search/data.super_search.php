 ...
 * @package		Solspace:Super Search
 * @author		Solspace DevTeam
 * @filesource	./system/expressionengine/third_party/super_search/data.super_search.php
 */
...
{	
	...
	function playa_is_installed()
    {
        ...
        if ( ee()->db->table_exists( 'exp_ff_fieldtypes' ) === FALSE )
        {
        	return FALSE;
        }
        ...
        $query	= ee()->db->query( "SELECT COUNT(*) AS count FROM exp_ff_fieldtypes WHERE class = 'playa' AND enabled = 'y'" );
	...
	function log_terms( $keywords = '' )
    {
        ...
        $sql_block = array();
       	...
       	if( count( $keywords ) > 0 ) 
       	{
       		...
       		{       			
       			$sql_block[] = "( '" . ee()->db->escape_str( $term ) . "', soundex('" . ee()->db->escape_str( $term ) . "'), " . ee()->config->item('site_id') . ", '" . ee()->localize->now . "', '". ee()->localize->now . "', 1 )";
       		}
       	}

       	$sql .= implode( ', ', $sql_block );

       	$sql .= " ON DUPLICATE KEY UPDATE last_seen = '" . ee()->localize->now . "', count = count + 1 , first_seen = IF( first_seen = 0, '". ee()->localize->now . "', first_seen ) ";
	...
	{	
		...
		ee()->db->query(" INSERT INTO exp_super_search_lexicon_log ( type, entry_ids, member_id, origin, action_date ) 
					VALUES ( 'build', '" . $entries_str . "', 0, 'manual', " . ee()->localize->now . " ) ");
	...
	function _load_fieldtypes()
	{
		...
		if( ! isset( $this->sess['lexicon']['fields'] ) )
		{
			...
			if( $third_party_fields )
			{
				...
				$query = ee()->db->query( " SHOW COLUMNS FROM exp_super_search_indexes WHERE Field IN( '" . implode( array_keys( $this->sess['index']['fields'] ), "','" ) . " ') ");
