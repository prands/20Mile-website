 ...
 * @package 	Solspace:Super Search
 * @author		Solspace DevTeam
 * @filesource	./system/expressionengine/third_party/super_search/mcp.super_search.php
 */
...
class Super_search_mcp extends Module_builder_super_search
{
    ...
    function __construct( $switch = TRUE )
    {
        ...
        $menu['module_cache_rules']	= array(
			...
			'title' => ee()->lang->line('cache_rules')
		);	*/
		...
		if (APP_VER < 2.0)
		{
			$search_log_path = BASE . '&C=admin&M=utilities&P=view_search_log';
		}		
		...
		if ( APP_VER >= 2.0) 
		{
		    $menu['module_search_utils']	= array(
				...
				'title' => ee()->lang->line('search_utils')
			);	
		}
		...
		$this->cached_vars['lang_module_version'] 	= ee()->lang->line('super_search_module_version');        
		$this->cached_vars['module_version'] 		= SUPER_SEARCH_VERSION;
        $this->cached_vars['module_menu_highlight']	= 'module_home';
        $this->cached_vars['module_menu']			= (APP_VER < 2.0) ? $this->equalize_menu($menu) : $menu;
    ...
    {    	
		...
		switch ( $_POST['type'] )
		{
			case 'character':
				...
				{
					$error[]	= ee()->lang->line('field_length_required');
				}
				elseif ( $_POST['length'] < 1 OR $_POST['length'] > 255 )
				{
					$error[]	= ee()->lang->line('char_length_incorrect');
				}
			...
			case 'varchar':
				...
				elseif ( $_POST['length'] < 1 OR $_POST['length'] > 255 )
				{
					$error[]	= ee()->lang->line('varchar_length_incorrect');
				}
		...
		$this->cached_vars['question']	= ee()->lang->line('edit_field_question');
		
		if ( isset( $check_truncation ) === TRUE )
		{
			$this->cached_vars['question']	= ee()->lang->line('edit_field_question_truncate');
		}
		...
		$this->add_crumb( ee()->lang->line( 'edit_field' ) );
    ...
    function edit_field_form( $message = '' )
    {
		...
		{
			$sql	= "SELECT field_id, field_name, field_label FROM " . $this->sc->db->fields . " WHERE field_id = '".ee()->db->escape_str( ee()->input->get_post('field_id') )."' LIMIT 1";
    ...
    function fields($message = '')
    {
		...
		$this->add_crumb( ee()->lang->line('fields') );
		$this->cached_vars['module_menu_highlight'] = 'module_fields';
	...
	function index( $message = '' )
    {
		...
		{
			$this->cached_vars['lang_no_searches_yet'] = ee()->lang->line('no_searches_yet');
		}
		else
		{
			$this->cached_vars['lang_no_searches_yet'] = str_replace( '%enable_link%', $this->base.'&method=search_options', ee()->lang->line('no_searches_yet_long') );
		}
		...
		if( APP_VER >= 2.0 ) 
		{
			...
			if( $query->num_rows() < 1 )
			{
				...
				$equery = ee()->db->query( " SELECT COUNT(*) as count FROM {$this->sc->db->channel_titles} " );
		...
		$this->cached_vars['loading_gif_url'] = ee()->config->item('theme_folder_url') . "third_party/super_search/images/indicator_dark.gif";

		$crumb	= ee()->lang->line( 'Homepage' );
	...
	function lexicon( $message = '' )
    {
		...
		if( ee()->input->get_post('build') == 'yes' )
		{
			if( ee()->input->get_post('ajax') == 'yes' )
			{
				...
				exit(ee()->javascript->generate_json( $this->cached_vars['response'] ) );
		...
		{
			$last_build = ee()->localize->set_human_time( $queryb->row('action_date') );

			$this->cached_vars['lexicon_build_lang'] = ee()->lang->line('lexicon_rebuild');

			$this->cached_vars['lexicon_last_build'] = ee()->lang->line('lexicon_last_built') . $last_build;// has never been built.';		
		}
		else
		{
			$this->cached_vars['lexicon_build_lang'] = ee()->lang->line('lexicon_build');

			$this->cached_vars['lexicon_last_build'] =  ee()->lang->line('lexicon_never_built');
		}

		$this->cached_vars['lexicon_build_url'] = $this->base.'&method=lexicon&build=yes&total='.$query->row('total_entries').'&batch=0';

		$this->cached_vars['success_png_url'] = ee()->config->item('theme_folder_url') . "third_party/super_search/images/success.png";
		...
		$crumb	= ee()->lang->line( 'manage_lexicon' );
	...
	function spelling( $message = '' )
    {
		...
		$crumb	= ee()->lang->line( 'manage_spelling' );
    ...
    {		
		...
		if ( ee()->input->post('use_ignore_word_list') !== FALSE )
		{
	        ...
	        {
				$errors[]	= ee()->lang->line('numeric_refresh');
	        }
			...
			$query		= ee()->db->query( "SELECT COUNT(*) AS count FROM exp_super_search_refresh_rules WHERE site_id = ".ee()->config->item('site_id') );
		...
		$crumb	= ee()->lang->line( 'manage_search_options' );
		...
		if ( $query->num_rows() > 0 )
		{
			...
			{
				$this->cached_vars['next_refresh']		= str_replace( '%n%', ee()->localize->set_human_time( $query->row('date') ), ee()->lang->line('next_refresh') );
			}			
		}		
		...
		if( APP_VER >= 2.0 )
		{
			...
			$query = ee()->db->query( " SELECT COUNT(entry_id) AS total_entries FROM {$this->sc->db->channel_titles} ");
	...
	function search_utils( $message = '' )
    {
		...
		$crumb	= ee()->lang->line( 'manage_search_utils' );
    ...
    {		
		...
		if( $this->check_yes( ee()->config->item('enable_search_log') ) ) 
		{
			$this->cached_vars['no_searches_recorded'] = ee()->lang->line('no_searches_recorded');
		}
		else
		{
			$this->cached_vars['no_searches_recorded'] = str_replace( '%enable_link%', $this->base.'&method=search_options', ee()->lang->line('no_searches_yet_long') );
		}
			...
			OR ee()->input->get_post('search_term') !== FALSE )
		{
			...
			else
			{
				foreach( $query->result_array() as $row ) 
				{
					...
					{				
						$row['first_seen_formatted'] = ee()->localize->set_human_time($row['first_seen']);
					...
					{
						$row['last_seen_formatted'] = ee()->localize->set_human_time($row['last_seen']);	
        ...
        foreach( $search_data as $row )
        {
			...
			$row['search_date_formatted'] = ee()->localize->set_human_time($row['search_date']);
    ...
    {
        require_once PATH_THIRD.'super_search/upd.super_search.php';
    ...
    function super_search_module_update()
    {
    	...
    	{
    		$this->add_crumb(ee()->lang->line('update_super_search'));
	...
	private function _add_styles( $include_all = FALSE, $cache_vars = FALSE )
	{
        ...
        if ( ! isset($ss_cache['assets']['super_search']) )
        {
			if( APP_VER < 2.0 )
			{
				$this->cached_vars['super_search_theme_styles'] = '<link rel="stylesheet" type="text/css" href="' . ee()->config->item('theme_folder_url') . 'third_party/super_search/css/super_search.css" />';
				
				if( $include_all )
				{
					$this->cached_vars['super_search_theme_scripts'] = '<script type="text/javascript" src="' . ee()->config->item('theme_folder_url') . 'third_party/super_search/js/jquery.animate-shadow-min.js"></script> <script type="text/javascript" src="' . ee()->config->item('theme_folder_url') . 'third_party/super_search/js/super_search.js"></script>';
				}
				else
				{
					$this->cached_vars['super_search_theme_scripts'] = '<script type="text/javascript" src="' . ee()->config->item('theme_folder_url') . 'third_party/super_search/js/super_search.js"></script>';
				}
			}
			else 
			{
		        ee()->cp->add_to_head( '<link rel="stylesheet" type="text/css" href="' . ee()->config->item('theme_folder_url') . 'third_party/super_search/css/super_search.css" />'); 

		        if( $include_all ) 
		        {
		        	ee()->cp->add_to_foot( '<script type="text/javascript" src="' . ee()->config->item('theme_folder_url') . 'third_party/super_search/js/jquery.animate-shadow-min.js"></script>');
			    }

		        ee()->cp->add_to_foot( '<script type="text/javascript" src="' . ee()->config->item('theme_folder_url') . 'third_party/super_search/js/super_search.js"></script>');
	...
	private function _theme_path()
	{
		if( APP_VER < 2.0 )
		{
			return ee()->config->item('theme_folder_url') . 'super_search/';
		}
		else
		{
			return ee()->config->item('theme_folder_url') . 'third_party/super_search/';
		}
	...
	{
		$year 		= ee()->lang->line('period_year');
		$month 		= ee()->lang->line('period_month');
		$day 		= ee()->lang->line('period_day');
		$hour 		= ee()->lang->line('period_hour');
		$min 		= ee()->lang->line('period_min');
		$sec 		= ee()->lang->line('period_sec');
		$postfix 	= ee()->lang->line('period_postfix');
		$ago 		= ee()->lang->line('period_ago');

		$etime = $ptime;

	    if ($etime < 1)  return ee()->lang->line('period_now');
	...
	private function _spelling_data() 
	{
		...
		$this->cached_vars['spelling_build_lang'] = ee()->lang->line('lexicon_build');
		...
		$this->cached_vars['suggestions_unknown_lang'] = str_replace( array('%i%', '%s%'), array( $this->cached_vars['spelling_unknown_count'], $plural ) , ee()->lang->line('spelling_unknown_line') );
	

		$plural = ( $this->cached_vars['spelling_known_count'] > 1 ? 's' : '' );

		$this->cached_vars['suggestions_known_lang'] = str_replace( array('%i%', '%s%'), array( $this->cached_vars['spelling_known_count'], $plural ) , ee()->lang->line('spelling_known_line') );
