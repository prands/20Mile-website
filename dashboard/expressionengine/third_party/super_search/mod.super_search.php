 ...
 * @package		Solspace:Super Search
 * @author		Solspace DevTeam
 * @filesource	./system/expressionengine/third_party/super_search/mod.super_search.php
 */
...
class Super_search extends Module_builder_super_search
{
	...
	function __construct ()
	{
		...
		if ( $this->database_version() == FALSE )
		{
			...
			trigger_error(ee()->lang->line('super_search_module_disabled'), E_USER_NOTICE);
		}
		...
		if ( $this->version_compare($this->database_version(), '<', SUPER_SEARCH_VERSION) )
		{
			...
			trigger_error(ee()->lang->line('super_search_module_out_of_date'), E_USER_NOTICE);
		}
		...
		{
			$edata = ee()->extensions->universal_call('super_search_extra_basic_fields', $this);
		}
	}
	...
	function _clear_cached()
	{
		...
		do
		{
			ee()->db->query(
				...
				WHERE date < ".( ee()->localize->now - ( $this->data->get_refresh_by_site_id( ee()->config->item( 'site_id' ) ) * 60 ) )."
		...
		do
		{
			ee()->db->query(" DELETE FROM exp_super_search_history
						WHERE search_date < ".( ee()->localize->now - ( 60 * 60 ) )."
						AND saved = 'n'
						AND cache_id NOT IN (
							SELECT cache_id
							FROM exp_super_search_cache )
	...
	function do_search_ct_cd( $q = array() )
	{
		...
		ee()->TMPL->log_item( 'Super Search: Starting prep query' );
		...
		{
			$where	.= ' AND t.entry_date < '.ee()->localize->now;
		}
		...
		{
			$where	.= ' AND (t.expiration_date = 0 OR t.expiration_date > '.ee()->localize->now.')';
		}
		...
		$tmpl_sites = $this->_prep_site_ids( ee()->TMPL->fetch_param('site') );
		
		$search_sites = array_intersect( $tmpl_sites, $passed_sites );
		
		if ( count( $search_sites ) == 0 ) return FALSE;
				
		ee()->TMPL->site_ids = $this->sess['search']['q']['site'] = $search_sites;
		...
		{
			$this->_fields( 'searchable', ee()->TMPL->site_ids );
		}		
		...
		{
			ee()->TMPL->log_item( 'Super Search: Ending cached do_search('.(microtime(TRUE) - $t).')' );
		...
		if ( ( $customfields = $this->_fields( 'searchable', ee()->TMPL->site_ids ) ) !== FALSE AND empty( $this->sess['search']['q']['field'] ) === FALSE )
		{
			// -------------------------------------
		...
		if ( ( $customfields = $this->_fields( 'searchable', ee()->TMPL->site_ids ) ) !== FALSE AND empty( $this->sess['search']['q']['search_in'] ) === FALSE )
		{
			// -------------------------------------
			//	Set a variable to control field searching. search-words-within-words set to no overrides default behavior and requires that searches return only for exact words not their conjugates.
			// -------------------------------------
		...
		{
			$arr	= ee()->extensions->universal_call( 'super_search_do_search_and_array', $this, array( 'and' => $and, 'not' => $not, 'or' => $or, 'subids' => $subids ) );
		...
		if ( $query->num_rows() == 0 AND ee()->extensions->active_hook('super_search_alter_ids') === FALSE )
		{
			$this->_cache( $this->sess( 'search' ), '', 0 );
		...
		ee()->TMPL->log_item( 'Super Search: Ending do_search_ct_cd('.(microtime(TRUE) - $t).' Results '.$query->num_rows().')' );
	...
	function _entries ( $ids = array(), $params = array() )
	{
		...
		foreach ( array( 'num' => 'limit' ) as $key => $val )
		{
			...
			if ( ! empty( ee()->TMPL->tagparams[ $val ] ) )
			{
			...
			if ( ! empty( ee()->TMPL->tagparams[ $key ] ) )
			{
	...
	function _fields( $channel = 'searchable', $site_ids = array() )
	{
		...
		if ( ! empty( ee()->extensions->version_numbers['Gypsy'] ) )
		{
			// -------------------------------------
			//	Begin SQL
			// -------------------------------------
    ...
    function forget_last_search()
    {
		...
		{
			$message	= ee()->lang->line( 'no_search_history_was_found' );
		...
		{
			$message	= ee()->lang->line( 'last_search_cleared' );
	...
	function cloud ()
	{
		...
		else // OR
		{
	        ...
	        if ( $start_on != '' )
	        {
	            $sql .= " AND last_seen >= '".ee()->localize->convert_human_date_to_gmt($start_on)."'";
	        }

	        if ( $stop_on != '' )
	        {
	            $sql .= " AND last_seen < '".ee()->localize->convert_human_date_to_gmt($stop_on)."'";
	        }
		...
		{
			$sql_a .= ( ctype_digit( ee()->TMPL->fetch_param('limit') ) === TRUE ) ? ' LIMIT '.ee()->TMPL->fetch_param('limit') : ' LIMIT 20';
        }
	...
	function _get_cat_group_ids()
	{
		...
		{
			$sql	= '/* Super Search fetch group ids */ SELECT group_id FROM exp_category_groups WHERE site_id IN ('.implode( ',', ee()->TMPL->site_ids ).')';
	...
	function _get_ids_by_category( $category = array() )
	{
		...
		$cat_name_query	= ( ee()->TMPL->fetch_param('category_indicator') !== FALSE AND ee()->TMPL->fetch_param('category_indicator') == 'category_url_title' ) ? 'c.cat_url_title': 'c.cat_name';
		...
		$where	= ' WHERE c.site_id IN ('.implode( ',', ee()->TMPL->site_ids ).')';
	...
	function _get_ids_by_category_like( $category = array() )
	{
		...
		$t	= microtime(TRUE);
		ee()->TMPL->log_item( 'Super Search: Beginning _get_ids_by_category_like()' );
		...
		if ( $query->num_rows() == 0 )
		{
			ee()->TMPL->log_item( 'Super Search: Ending _get_ids_by_category_like() ('.(microtime(TRUE) - $t).')' );
			return FALSE;
		}
	...
	function _get_uri($method = 'fetch')
	{
		...
		if ( APP_VER < 2.0 )
		{
			...
			{				
				return rtrim( ee()->security->xss_clean( $this->_sanitize( trim_slashes( $_SERVER['REQUEST_URI'] ) ) ), '/' ) . '/';
			}
			else
			{				
				return rtrim( ee()->security->xss_clean( $this->_sanitize( trim_slashes( $GLOBALS['uri'] ) ) ), '/' ) . '/';
			}
		...
		else
		{
			...
			else
			{
				...
				return rtrim( ee()->security->xss_clean( $this->_sanitize( trim_slashes( ee()->uri->uri_string() ) ) ), '/' ) . '/';	
			}	
		}
	}
	...
	function _highlight_keywords( $str = '' )
	{
		...
		if ( $str == '' OR 
			ee()->TMPL->fetch_param('highlight_keywords') == '' OR
			ee()->TMPL->fetch_param('highlight_keywords') == 'no' OR 
			( empty( $this->sess['search']['q']['keywords']['or'] ) === TRUE AND 
			  empty( $this->sess['search']['q']['keywords']['and'] ) === TRUE) ) return $str;
			
		$tag	= 'em';
		
		if ( ee()->TMPL->fetch_param('highlight_keywords') !== FALSE AND ee()->TMPL->fetch_param('highlight_keywords') != '' )
		{
			switch ( ee()->TMPL->fetch_param('highlight_keywords') )
			{
				case 'b':
					$tag	= 'b';
					break;
...
EVIL;
		...
		ee()->TMPL->log_item( 'Super Search: Ending highlight_keywords('.(microtime(TRUE) - $t) );
	...
	function _parse_date_to_timestamp( $date = '', $prefix = '', $full_day = FALSE )
	{
		...
		switch ( count( $thedate ) )
		{
			...
			case 2:	// We have year only
				$day	= ( $full_day === FALSE ) ? $day: ee()->localize->fetch_days_in_month( $month, $thedate[0].$thedate[1] );
				$return	= $prefix .mktime( $hour, $minute, $second, $month, $day, $thedate[0].$thedate[1] );
				break;
			case 3:	// We have year and month
				$day	= ( $full_day === FALSE ) ? $day: ee()->localize->fetch_days_in_month( $thedate[2], $thedate[0].$thedate[1] );
	...
	function _parse_no_results_condition( $q = array() )
	{
		...
		if ( strpos( ee()->TMPL->template, LD.'super_search_suggestion'.RD ) !== FALSE )
		{
			if ( !isset( $q['suggestion'] ) ) 
			{
				ee()->TMPL->template	= str_replace( LD.'super_search_suggestion'.RD, '', ee()->TMPL->template );
			}
			else
			{
				ee()->TMPL->template	= str_replace( LD.'super_search_suggestion'.RD, $q['suggestion'], ee()->TMPL->template );
			}
		}
	...
	function _parse_template_vars()
	{
		...
		foreach ( $this->basic as $key )
		{
			if ( strpos( ee()->TMPL->template, $p . $key ) === FALSE ) continue;
		...
		if ( ( $fields = $this->_fields( 'searchable', ee()->TMPL->site_ids ) ) !== FALSE )
		{
			...
			{
				if ( strpos( ee()->TMPL->template, $p.$key ) === FALSE 
					AND strpos( ee()->TMPL->template, $p.'exact'.$this->modifier_separator.$key ) === FALSE 
					AND strpos( ee()->TMPL->template, $p.$key.$this->modifier_separator.'exact' ) === FALSE 
					AND strpos( ee()->TMPL->template, $p.$key.$this->modifier_separator.'empty' ) === FALSE 
					AND strpos( ee()->TMPL->template, $p.$key.$this->modifier_separator.'from' ) === FALSE 
					AND strpos( ee()->TMPL->template, $p.$key.$this->modifier_separator.'to' ) === FALSE ) continue;
		...
		{
			$parse	= ee()->extensions->universal_call( 'super_search_parse_template_vars_end', $this, $parse, $p );
		}
		...
		ee()->TMPL->template	= ee()->functions->prep_conditionals( ee()->TMPL->template, $parse );
		
		foreach ( $parse as $key => $val )
		{
			if ( in_array( $key, array( $p.'date'.$this->modifier_separator.'from', $p.'date'.$this->modifier_separator.'to' ) ) === TRUE )
			{
				if ( strpos( ee()->TMPL->template, 'format=' ) !== FALSE )
				{					
					if ( preg_match_all( "/" . LD . $key . "\s+format=[\"'](.*?)[\"']" . RD . "/s", ee()->TMPL->template, $format ) )
					{
						...
						foreach ( $format[0] as $k => $v )
						{
							...
							{							
								ee()->TMPL->template	= str_replace( $format[0][$k], $this->_parse_date( $format[1][$k], $this->_parse_date_to_timestamp( $val, '', $full_day ) ), ee()->TMPL->template );
							}
						}
					}
				}
			...
			{
				ee()->TMPL->template	= str_replace( LD.$key.RD, $val, ee()->TMPL->template );
			}
			else
			{
				ee()->TMPL->template	= str_replace( LD.$key.RD, str_replace( $this->spaces, ' ', $val ), ee()->TMPL->template );
			}			
		}
	}
	...
	{		
		...
		if ( ee()->TMPL->fetch_param('author_indicator') !== FALSE AND in_array( ee()->TMPL->fetch_param('author_indicator'), array( 'author_id', 'member_id', 'screen_name', 'username' ) ) === TRUE )
		{
		...
		{		
			...
			{
				$author['not'][$key]	= ee()->db->escape_str( $val );
			}
			...
			if ( isset( $this->sess['search']['q']['partial_author'] ) === TRUE AND $this->check_yes( $this->sess['search']['q']['partial_author'] ) == TRUE AND $indicator != 'member_id' AND $indicator != 'author_id')
			{
				...
				{
					$sql .= '( ' . $indicator . ' NOT LIKE \'%' . ee()->db->escape_str( $single ) . '%\' ) AND ';
				}
		...
		{		
			...
			{
				$author['or'][$key]	= ee()->db->escape_str( $val );
			}
			...
			if ( isset( $this->sess['search']['q']['partial_author'] ) === TRUE AND $this->check_yes( $this->sess['search']['q']['partial_author'] ) == TRUE AND $indicator != 'member_id' AND $indicator != 'author_id' )
			{
				...
				{
					$sql .= '( ' . $indicator . ' LIKE \'%' . ee()->db->escape_str( $single ) . '%\' ) OR ';
				}
	...
	function _prep_channel( $channel_string = '' )
	{
		...
		if ( ee()->TMPL->fetch_param( $this->sc->db->channel_id ) !== FALSE AND ee()->TMPL->fetch_param( $this->sc->db->channel_id ) != '' )
		{
			$channel_ids	= explode( '|', ee()->TMPL->fetch_param( $this->sc->db->channel_id ) );
		}
		...
		if ( ! empty( $channel_string ) )
		{
			...
			if ( ee()->TMPL->fetch_param( 'channel' ) !== FALSE AND ee()->TMPL->fetch_param( 'channel' ) != '' )
			{
				...
				else
				{
					...
					$channel_names['or']	= array_intersect( explode( "|", ee()->TMPL->fetch_param( 'channel' ) ), $channel_names['or'] );
	...
	function _prep_group( $group = array() )
	{
		...
		{		
			...
			{
				$group['not'][$key]	= ee()->db->escape_str( $val );
			}
		...
		{		
			...
			{
				$group['or'][$key]	= ee()->db->escape_str( $val );
			}
	...
	function _prep_keywords( $keywords = '' , $inclusive = FALSE, $type = '')
	{
		...
		foreach ( $keywords as $phrase )
		{
			...
			$temp	= explode( ' ', trim( ee()->db->escape_str( $phrase ) ) );
		...
		if ( ee()->config->item('enable_fuzzy_searching') == 'y' AND $type == 'keywords' AND APP_VER >= 2.0 )
		{
    ...
    function _prep_fuzzy_phonetics( $arr = array() )
    {
    	...
    	foreach( $arr['or'] as $or )
    	{
    		...
    		$terms[] = " SOUNDEX('".ee()->db->escape_str( $or )."') ";
    	}
    	...
    	foreach( $arr['and'] as $and )
    	{
    		...
    		$terms[] = " SOUNDEX('".ee()->db->escape_str( $and )."') ";
    	}
    ...
    function _prep_fuzzy_plurals( $arr = array() )
    {
    	...
    	foreach( $arr['or'] as $or )
    	{	
    		$terms[] = ee()->db->escape_str( $or );
    	}

    	// Handle 'ye ANDs
    	foreach( $arr['and'] as $and )
    	{
    		$terms[] = ee()->db->escape_str( $and );
    	}
    ...
    function _prep_fuzzy_spelling( $arr = array() )
    {
    	...
    	foreach( $arr['or'] as $or )
    	{	
    		$terms[ $or ] = ee()->db->escape_str( $or );
    	}

    	// Handle 'ye ANDs
    	foreach( $arr['and'] as $and )
    	{
    		$terms[ $and ] = ee()->db->escape_str( $and );
    	}
	...
	function _prep_order( $order = '' )
	{
		...
		{
			$arr	= ee()->extensions->universal_call( 'super_search_prep_order', $this, $arr );
		}
		...
		if ( $type == 'and' AND empty( $keywords['and'] ) === FALSE )
		{
			...
			foreach ( $keywords['and'] as $val )
			{
				...
				{				
					...
					{
						$temp[]	= $field." REGEXP '".str_replace( $this->wildcard, '[a-zA-Z0-9]+', ee()->db->escape_str( $val ) ) ."'";
					}
					else
					{
		...
		if ( $type == 'not' AND empty( $keywords['not'] ) === FALSE )
		{
			...
			foreach ( $keywords['not'] as $val )
			{
				...
				{					
					...
					elseif ( $allow_wildcards AND stripos( $val, $this->wildcard ) !== FALSE )
					{
						$temp[]	= $field." NOT REGEXP '".str_replace( $this->wildcard, '[a-zA-Z0-9]+', ee()->db->escape_str( $val ) ) ."'";
					}
					else
					{
						$temp[]	= $field." NOT LIKE '%".ee()->db->escape_str( $val )."%'";
					}
				}
			}
		...
		if ( $this->relevance_frequency > 0 )
		{
			...
			{					
				$fsql = " SELECT term, entry_count FROM exp_super_search_terms 
							WHERE term IN ('" . implode( "','", ee()->db->escape_str($keywords) ) . "') ";
				
				$fquery = ee()->db->query( $fsql );
	...
	{		
		...
		if ( $post === TRUE )
		{
			return ee()->output->show_user_error( 'general', $cond['message'] );
		}
	...
	function save_search()
	{
        ...
        {
        	return $this->_return_message( $post, '', array( 'message' => ee()->lang->line('search_not_found') ) );
        }
        ...
        if ( ( isset( ee()->TMPL ) === TRUE AND ee()->TMPL->fetch_param('delete') !== FALSE AND ee()->TMPL->fetch_param('delete') == 'yes' ) OR strpos( ee()->uri->uri_string, '/delete' ) !== FALSE OR ee()->input->get_post('delete_mode') == 'yes' )
        {
        	...
        	if ( $post === FALSE )
        	{
				return $this->_return_message( $post, ee()->TMPL->tagdata, array( 'failure' => FALSE, 'success' => TRUE, 'message' => ee()->lang->line('search_successfully_deleted') ) );
        	}
        	else
        	{
				ee()->functions->redirect( $return );
        ...
        {
        	return $this->_return_message( $post, '', array( 'message' => ee()->lang->line('missing_name') ) );
        }
        elseif ( preg_match("/[^a-zA-Z0-9\_\-\.\s]/", $search_name ) )
        {
        	// return $this->_return_message( $post, '', array( 'message' => ee()->lang->line('invalid_name') ) );
        ...
        {
        	return $this->_return_message( $post, '', array( 'message' => ee()->lang->line('duplicate_name') ) );
        }
        ...
        if ( $post === FALSE )
        {
			return $this->_return_message( $post, ee()->TMPL->tagdata, array( 'failure' => FALSE, 'success' => TRUE, 'message' => ee()->lang->line('search_successfully_saved') ) );
        }
        else
        {
			ee()->functions->redirect( $return );
	...
	function _save_search_to_history( $cache_id = 0, $results = 0, $q = '' )
	{
        ...
        $arr	= array(
        		...
        		'search_name'	=> ee()->lang->line('search'),
        		'results'		=> $results,
        		'hash'			=> $this->hash,
        		'query'			=> $q,
			);
	...
	function search()
	{
		...
		if ( ( $member_id = ee()->session->userdata('member_id') ) === 0 )
		{
			if ( ( $cookie_id = $this->_get_users_cookie_id() ) === FALSE )
			{
				return $this->_strip_variables( ee()->TMPL->tagdata );
			}
		}
		...
		foreach ( $query->result_array() as $row )
		{
			...
			$tagdata	= ee()->functions->prep_conditionals( $tagdata, array_merge( $row, $vars ) );
	...
	{		
		...
		if ( isset(ee()->TMPL->module_data['Rating']) && ! isset( $this->sess['fields']['searchable']['rating'] ) )
		{
		...
		if ( $term != '' )
		{
			...
			foreach( $splitters AS $split )
			{
				...
				{			
					...
					{
						if ( $single != '' )	$terms[] = ee()->db->escape_str( $single );
					}		
				}
			}
