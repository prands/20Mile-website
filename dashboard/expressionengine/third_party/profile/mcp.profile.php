...
class Profile_mcp
{
	...
	{
		$this->EE->cp->set_variable('cp_page_title', lang('profile_module_name'));
	...
	public function installation()
	{
		...
		$vars = array(
			...
			'template_errors' => ($this->EE->session->flashdata('message_failure')) ? $this->EE->session->flashdata('message_failure') : array(),
			'templates_installed' => ($this->EE->session->flashdata('message_success')) ? $this->EE->session->flashdata('message_success') : array(),
		);
			...
			createProfiles: {
				...
				start: function(){
					$.post(
						...
						function(data){
							$.profile.createProfiles.memberCount = Number(data);
							console.log(data);
	...
	public function create_profiles()
	{
		...
		foreach ($query->result_array() as $row)
		{
			$this->EE->profile_model->create_profile($row, $row['member_id']);
		}
