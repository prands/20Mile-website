...
class Profile_model extends CI_Model
{
	...
	protected static $native_fields = array(
		'url',
		'location',
		'occupation',
		'interests',
	...
	public function get_native_fields($custom_fields_only = FALSE)
	{
		...
		$native_fields = $member_fields;
		
		foreach (self::$native_fields as $field)
		{
			$native_fields[] = $field;
		}
	...
	public function get_member_data($member_id)
	{
		...
		foreach ($this->get_native_fields(TRUE) as $field_id => $field_name)
		{
			$this->db->select("`m_field_id_$field_id` AS `$field_name`", FALSE);
		}
		
		$query = $this->db->select('username, screen_name, email, email AS email_confirm')
		...
		if ($query->num_rows() > 0)
		{
			$cache[$member_id] = $vars = $query->row_array();
		}
	...
	public function validate_entry_id($entry_id, &$member_id = NULL)
	{
		...
		if ($this->session->userdata('group_id') != 1 || $this->session->userdata('can_admin_members') !== 'y')
		{
	...
	public function get_channel_info($key = FALSE)
	{
		...
		if (is_null($cache))
		{
			...
			if ($this->channel_id())
			{
				$query = $this->db->select('channels.channel_name, channels.site_id, sites.site_name')
	...
	public function get_profile_data($member_id)
	{
		static $cache;
	...
	public function send_email($email, $template, $variables = array())
	{
		...
		return $this->email->from($this->config->item('webmaster_email'), $this->config->item('webmaster_name'))
				   ->to($email)
				   ->subject($subject)
				   ->message($message)
				   ->set_wordwrap()
	...
	public function build_action_url($action, $params = array())
	{
		...
		{
			$params['return'] = ($this->TMPL->fetch_param('return')) ? $this->TMPL->fetch_param('return') : $this->uri->uri_string();
		}
		
		return $this->functions->fetch_site_index(0, 0).QUERY_MARKER
			.'ACT='.$this->functions->fetch_action_id('Profile', $action.'_action')
