...
class Addon_builder_super_search_code_pack {
	...
	public function Addon_builder_super_search_code_pack ($name='')
	{
		//path to this folder
		...
		if (APP_VER < 2.0)
		{
			...
			if (($parent_class == 'Extension_builder_super_search_code_pack' OR
				 $super_parent_class == 'Extension_builder_super_search_code_pack') AND
		...
		else
		{
			...
			{
				if ( ! class_exists('Addon_builder_data_super_search_code_pack'))
				{
					require_once $this->aob_path . 'data.addon_builder.php';
				}

				$this->data = new Addon_builder_data_super_search_code_pack($this);
			}
		...
		//--------------------------------------------
		// documentDOM_super_search_code_pack instantiated, might move this.
		//--------------------------------------------

		if (REQ == 'CP' AND file_exists($this->aob_path . 'document_dom.php'))
		{
			if ( ! class_exists('documentDOM_super_search_code_pack'))
			{
				require_once $this->aob_path . 'document_dom.php';
			}

			$this->document = new documentDOM_super_search_code_pack();
		}
	...
	// END Addon_builder_super_search_code_pack()
