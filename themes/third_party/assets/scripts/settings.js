...
function onBucketSelectChange()
{
    ...
    $('#bucket_location').val(_sel.attr('data-location'));
    $('#bucket_url_prefix').val(_sel.attr('data-url-prefix'));
}
...
$('.refresh_buckets').live('click', function (e)
{
    ...
    $('input.setting_field').each(function ()
    {
        params[$(this).prop('name')] = $(this).val();
    });
    $('#buckets').html('<div class="assets-loading"></div>');
    $.post(Assets.siteUrl, params, function (data)
    {
        $('#buckets').html(data);
        $bucketSelect = $('#buckets > select');
    ...
    if (sources_to_index.length > 0)
    {
        $.post(Assets.siteUrl, {ACT: Assets.actions.get_session_id}, function (data)
        {
            ...
            var queue = new Assets.AjaxQueueManager(10, function ()
            {
                ...
                {
                    data = $.evalJSON(data);
                    if (typeof missing_folders.length > 0 || typeof data.files != "undefined")
                    {
                        ...
                        if (missing_folders.length > 0)
                        {
                            ...
                            {
                                html += '<div><label><input type="checkbox" checked="checked" class="delete_folder" value="' + missing_folders[i].folder_id + '" /> ' + queue.missing_folders[i].folder_name + '</label></div>';
                            }
                            html += '</div>'
                        }
